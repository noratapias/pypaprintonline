...
privileged aspect UserController_Roo_Controller_Json {
    ...
    @ResponseBody
    public ResponseEntity<String> UserController.jsonFindUsuariosByUsernameEquals(@RequestParam("username") String username) {
        HttpHeaders headers = new HttpHeaders();
        headers.add("Content-Type", "application/json; charset=utf-8");
        return new ResponseEntity<String>(Usuario.toJsonArray(Usuario.findUsuariosByUsernameEquals(username).getResultList()), headers, HttpStatus.OK);
    }
